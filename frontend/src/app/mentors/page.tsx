...
export default function MentorsPage() {
  ...
  return (
    <main className="min-h-screen">
      {/* ═══════════════════════════════════════════════════════════
          HERO SECTION — White & Theme-Adaptive Glassmorphism
          ═══════════════════════════════════════════════════════════ */}
      <section className="relative overflow-hidden bg-gradient-to-b from-brand-light/40 via-white to-white dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 border-b border-gray-100 dark:border-slate-800 min-h-[560px] lg:min-h-[640px] flex items-center justify-center py-16 sm:py-20 px-4 transition-colors">
        ...
        <ScrollReveal className="relative z-20 max-w-3xl mx-auto px-4 text-center">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 backdrop-blur-xl bg-brand-light/80 dark:bg-white/[0.08] border border-brand/20 dark:border-white/[0.15] rounded-full mb-6 shadow-md shadow-brand/5">
            ...
            <span className="text-brand dark:text-white/80 text-xs sm:text-sm font-semibold">{mentors.length} Industry Experts Available</span>
          </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 self-end lg:self-auto">
              <div className="flex items-center gap-2">
                ...
                <select
                  value={sortBy}
                  onChange={(e) => setSortBy(e.target.value as "rating" | "learners" | "experience" | "name")}
                  ...
                  <div className="flex -space-x-2.5 overflow-hidden">
                    {mentors.slice(0, 5).map((m) => (
                      <div key={m.id} className="inline-block h-8 w-8 rounded-full ring-2 ring-white dark:ring-slate-900 overflow-hidden relative">
                        <Image src={m.avatar || "/images/mentor-sarah-chen.jpg"} alt={m.name} fill className="object-cover" />
                      </div>
                    ))}
                  </div>
